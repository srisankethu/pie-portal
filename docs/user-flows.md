...
| PUT | `/api/v1/data/catalog/companies/{connection_id}/catalogues/{catalogue_key}/sources/{source_key}/decoding` | owner | Save one file's decoding config — which of its columns hold the record id, description and grade, and what decodes it: either a shipped rule set or a decoder built for this file, never both. Re-reads the stored bytes and refuses columns the file lacks, a rule set the engine does not ship, or a decoder that does not survive a re-freeze, so a config that cannot build is never stored; a config naming neither is saved but the file waits, by name. A decoder arrives as the artifact a proposal returned plus the `bindings` and `decimal` a person chose — patterns are not editable through this |
| POST | `/api/v1/data/catalog/companies/{connection_id}/catalogues/{catalogue_key}/sources/{source_key}/propose-decoder` | owner | Read one stored file and propose a decoder built for it alone: its descriptions clustered into shapes, a pattern induced per shape and validated over every row, each captured group measured, and the groups the file's own text settles named. The rest come back with their evidence and the slots they could be. Proposes and returns; saves nothing |
...
| GET | `/api/v1/insight/quote-book/{quote_ref}/lines` | signed-in | What was on one ERP quote, in the order the ERP wrote it; scoped through the book itself so a quote the reader may not see is a 404, and `lines_held` distinguishes a quote with no lines from one whose breakdown has not been pulled |
...
| POST | `/api/v1/quote-diagnosis/erp-quote/{quote_ref}` | signed-in, narrowed to the accounts the reader holds | Diagnose a quote the ERP already issued, as of the day it was raised. Takes no lines and no date: both are read from the document, which is what lets it answer for quotes older than `/assess` will — a date a caller supplies is a date a caller can walk, and `erp_quotes` is written by the sync and by no endpoint. Records nothing; 404 rather than 403 on a quote the reader may not see |
...
| GET | `/api/v1/quotes/{quote_id}/item-search` | signed-in | Find an item by hand: this company's decoded catalogue and its synced ERP item master, searched together and reported separately. Carries no price, cost or margin. Each half says whether it could be searched, so an unsearchable catalogue never reads as "no such item" |
