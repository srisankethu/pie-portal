...
def decorate_products(session: Session, organization_id: str, *,
                      ...
                      rule_set: Optional[Path] = None
                      ) -> DecorationReport:
    """Decode this organization's products and store what came back.

    ``rule_set`` is the pack these names are decoded through, and there is no
    default here on purpose. ``decode_names`` used to reach for
    ``settings.PIE_PACK`` itself; it now requires the caller to name one,
    because decoding one export through another's grammars reports that rule
    set's coverage of this file and calls it a finding. Absent, nothing is
    decoded and ``decoded_name_unavailable`` says so — UNKNOWN, not zero, which
    is the same answer this module gives for an unloaded catalogue.

    Both callers name one: the CLI takes ``--rule-set`` and the sync phase
    passes the organization layer. See ``master_health.geometry.decode_names``.
