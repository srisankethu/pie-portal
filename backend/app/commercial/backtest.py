...
class BacktestReport:
    ...
    #: Lines where replaying today's policy does not reproduce the verdict the
    #: row was actually stamped with. Expected for rows priced under an older
    #: policy — a ``ci_`` hash cannot be resolved back to the values it stood
    #: for, so the harness cannot rebuild the policy those rows were judged by.
...
def run(session: Session, org: str, *, min_margin: float,
    ...
    for row in session.scalars(stmt):
        ...
        before = verdict_for(quantity=quantity, quoted_unit_price=price,
                             unit_cost=cost, as_of=row.as_of, th=baseline)
        after = verdict_for(quantity=quantity, quoted_unit_price=price,
                            unit_cost=cost, as_of=row.as_of, th=variant)

        # The row records what actually happened. Where replaying today's policy
        # does not reproduce it, the row was judged by a policy this harness
        # cannot rebuild, and saying so is the honest answer.
...
def main() -> None:
    ...
    if report.baseline_disagreements:
        print(f"{report.baseline_disagreements} line(s) were priced under a "
              "policy this replay cannot rebuild, so their baseline verdict "
              "does not match what was recorded. Read the counts above as "
              "covering the rest.", file=sys.stderr)
