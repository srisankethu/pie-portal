...
def list_decisions(
    ...
    t0 = time.time()
    logger.info("[decisions:list] START type=%s status=%s org=%s",
                type, status_filter, principal.organization_id)
    ...
    for kind, model in masters.items():
        if any(d.subject_entity_type == kind for d in rows):
            ...
            t_idx_end = time.time()
            indexes[kind] = idx
            logger.info("[decisions:list] Loaded index for %s: %.1fms, size=%s",
                        kind, (t_idx_end-t_idx_start)*1000, len(idx))
    ...
    if include_detail:
        ...
        t7 = time.time()
        logger.info("[decisions:list] Loaded %s with details: %.1fms",
                    len(result), (t7-t6_detail)*1000)
...
def get_decision_detail(
    ...
    t0 = time.time()
    logger.info("[decisions:detail] START decision_id=%s org=%s",
                decision_id, principal.organization_id)
...
def get_decisions_bulk_detail(
    ...
    """
    t0 = time.time()
    logger.info("[decisions:bulk-detail] START with %s ids org=%s",
                len(decision_ids), principal.organization_id)
    ...
    t1 = time.time()
    logger.info("[decisions:bulk-detail] Loaded %s/%s details: %.1fms",
                len(result), len(decision_ids), (t1-t0)*1000)
    return result
