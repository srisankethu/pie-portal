...
# ``decode_names`` requires the rule set to be named — there is no default, so
# that a diagnostic cannot decode one export through another's grammars. Both
# production callers name the organization layer (the CLI's --rule-set default,
# and the sync phase), and so does every test below: what is under test here is
# the store, not which pack reads a Zoho material master.
