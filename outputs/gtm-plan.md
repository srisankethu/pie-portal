...
A candidate must clear all six.

| Criterion | Threshold | Why |
|---|---|---|
| SKU count | **> 5,000 active items** | A good salesperson holds a smaller catalogue in memory |
| Product complexity | Codes carry **encoded geometry** — ISO 1832 inserts, thread forms, bearing designations | An arbitrary part number has nothing to decode |
...
- **Commodity distributors** — high SKU count, no encoded geometry, price competition only.
...
**Ring 3 — adjacent categories**: bearings, abrasives, power transmission, MRO.

**The decision: geography before category.** Geography reuses the pack asset intact
and needs a connector that is already built. Category requires rebuilding that asset
against unfamiliar physics — bearing designations share nothing with carbide grades —
and must wait until the cost of building a pack is measured and falling. Categories
first spends the most engineering to prove the least.

---

## 3. The internal deployment — proof, and its limits

**What it proves.** The platform survives a real, messy book of business rather than a
demo fixture: 15,028 items, ₹32.7M of stock at selling price, three legal entities,
live data. Its users are not the author and would abandon it if it were slower than
the spreadsheet.

**What it does not prove.** Every requirement was validated by asking the founder.
There is no evidence anyone will *pay*, none that the workflow survives a sales culture
the founder does not control, and none that onboarding works when the person doing it
does not already know the answer. All three entities are on Zoho in one category, so
neither the seven-connector nor the cross-category claim has been tested.
...
**What PIE must show.** Coverage of their top-two manufacturers above **60%** of
stocked items `[ASSUMPTION]`; every quoted line resolved or honestly abstained; a
non-null ATTRIBUTED figure.
...
below your floor, what would that be worth per month — and what budget does it come
out of?"* It forces a number and a budget line in one answer, and the budget line
matters more: a figure with no home is a compliment, not a forecast.
...
12–15 to clear. Start the **Sandvik pack** and instrument the hours — the
highest-value work of the quarter, and it answers §6. Fix the `README.md` line that
contradicts sheet 13.
...
customer drives onboarding so the §3 test is real. Finish the Sandvik pack; publish
...
2. **Catalogue coverage per new customer**, stocked items, measured as the 21.6%
   internal census was. Predicts whether a pilot can pass before it starts.
