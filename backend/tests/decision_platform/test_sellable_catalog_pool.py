...
from app.attributes import CATALOGUE_LINK, DECODED_NAME, decorate_products
...
def _resolve(text: str, pool: Any = None):
    ...
    ps._resolution_cache.clear()
    return pie_service.resolve(text, bands=_bands(), pool=pool)
...
def _catalogue_namespace() -> str:
    ...
    with open(ensure_catalog(), encoding="utf-8") as fh:
        for line in fh:
            if line.strip():
...
def test_the_book_is_sellable_because_it_is_in_the_pool_not_because_it_is_the_book(session):
    ...
    *through* ``_sellable_namespaces`` rather than around it.
    ...
    pie_service.warm()
    args = pie_service._make_args(BOOK_REQUEST)
    sources = [*pie_service._sources, pool]

    open_result, _ = pie_service._mod.run(args, sources)
    ...
    args.sellable_namespaces = _catalogue_namespace()
    restricted, _ = pie_service._mod.run(args, sources)
...
def test_the_pool_fingerprint_is_in_the_resolution_cache_key(session):
    ...
    keyed_without = pie_service._cache_key(BOOK_REQUEST, None, None, None)
    keyed_with = pie_service._cache_key(BOOK_REQUEST, None, None, pool)
    assert keyed_with != keyed_without

    _write(session, row.product_id, "flute_count", "4", 4.0, "HUMAN")
    moved = sellable.sellable_pool_for(session, ORG)
    assert pie_service._cache_key(BOOK_REQUEST, None, None, moved) != keyed_with
...
def test_a_pool_that_cannot_be_fingerprinted_is_never_cached():
    ...
    assert pie_service._cache_key("x", None, None, object()) is None
    assert pie_service._cache_key("x", None, None, None) is not None
...
def test_a_drill_in_the_book_is_not_offered_for_an_endmill_request(session):
    ...
    """
    from app.attributes import decorate_products
