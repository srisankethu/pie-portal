 ...
 *
 * The hero's decision card and the note in the plans section describe one
 ...
 *
 * So this re-derives them. It fails if somebody edits a figure on the card and
 * not the note, or picks a floor that its own cost does not produce.
 *
 * The half of this file that checked a price list is gone with the list. The
 * one rule that outlived it — a visitor outside India must find no rupee
 * figure anywhere in a served document — is not weaker for that: the worked
 * card is denominated too, and `prerender.test.tsx` still holds every built
 * page to it.
 */
...
describe("the module the price list used to live in", () => {
  it("exports nothing that states what a plan costs", async () => {
    // The site's rule, asserted at its source rather than only in the markup:
    // what a business pays is settled in a conversation, so nothing here may
    // grow back into a figure a panel can print. A named export returning
    // "$1,950" would sail past every other test in this suite.
    const module = await import("./worked-example");
    const names = Object.keys(module);
    expect(names).not.toContain("tierPrice");
    expect(names).not.toContain("catalogBuildPrice");
    for (const [name, value] of Object.entries(module)) {
      if (typeof value !== "string") continue;
      expect(value, `${name} reads as a price`).not.toMatch(/[$₹]\s?\d/);
    }
  });
});
