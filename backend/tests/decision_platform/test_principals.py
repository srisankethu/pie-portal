"""Whose product an item is: a bill first, the item's brand second.
...
attributable to nobody. The item master's brand has no horizon but is missing
...
* an **ambiguous brand matches nothing** — a wrong match silently merges two
  principals, which is far worse than an extra column with the right name;
* an **unmatched brand is still a principal**, not a dropped row, or the
  fallback would fail to do the one job it exists for;
* the **purchase side never sees a brand**, so a target on what this book buys
...
def test_a_purchase_bill_beats_the_item_masters_brand():
    """The bill is a transaction; the brand is an attribute somebody typed.

    Where they disagree the bill is right about who was paid — which is the
    question — even though the brand may be right about whose logo is on the box.
    """
...
def test_the_brand_answers_where_no_bill_reaches():
    ...
    the brand this item would be attributed to nobody at all.
    """
    resolved = pr.resolve_all({"p1": "Kennametal India Limited"}, [], VENDORS)
    assert resolved["p1"].principal_id == "v_ken"
    assert resolved["p1"].source == pr.BY_BRAND
...
# ── matching a brand to a vendor, timidly ────────────────────────────────────
...
def test_punctuation_is_not_identity_either():
    """``YG1`` and ``YG-1`` are one brand; the hyphen is typing, not fact."""
...
def test_a_short_brand_matches_a_longer_vendor_name_by_prefix():
    """``NOGA`` is the brand; the vendor row is the trading entity."""
    assert pr.match_brand("NOGA", VENDORS) == ("v_noga", "noga")
    assert pr.match_brand("YG1", VENDORS) == ("v_yg", "yg1")


def test_an_ambiguous_brand_matches_nothing_rather_than_guessing():
    ...
    assert pr.match_brand("NOGA", ambiguous) is None
...
def test_two_vendor_rows_with_one_trading_name_resolve_to_neither():
    ...
    assert pr.match_brand("Kennametal India", duplicated) is None


def test_a_brand_too_short_to_be_evidence_does_not_prefix_match():
    """Two characters would let a brand claim any vendor beginning with them."""
    assert pr.match_brand("YG", {"v_yg": "YG-1 Industries"}) is None
    # An exact match on a short name is still fine — it is not an inference.
    assert pr.match_brand("YG", {"v_yg": "YG"}) == ("v_yg", "yg")


@pytest.mark.parametrize("brand", ["", "   ", None, "!!!", "---"])
def test_a_brand_with_nothing_in_it_matches_nothing(brand):
    assert pr.match_brand(brand, VENDORS) is None


def test_an_unmatched_brand_is_still_a_principal():
    ...
    got = resolved["p1"]
    assert got.known and got.source == pr.BY_BRAND
    assert got.principal_id == "brand:emugefranken"
    assert got.name == "Emuge Franken"          # as typed, not as normalised
    assert got.is_brand_only


def test_a_brand_that_matched_a_vendor_is_not_flagged_as_brand_only():
    """``is_brand_only`` asks whether a real supplier row backs this principal,
    not how the item reached it — a matched brand names a vendor this book has
    a record for, and a screen counting unbacked principals must not count it."""
    resolved = pr.resolve_all({"p1": "NOGA"}, [], VENDORS)
    assert resolved["p1"].source == pr.BY_BRAND
    assert not resolved["p1"].is_brand_only


def test_one_brand_is_matched_once_however_many_items_carry_it(monkeypatch):
    """A catalogue has thousands of items and, on the live masters, six brands."""
    calls = {"n": 0}
    real = pr.match_brand

    def counting(brand, vendors):
        calls["n"] += 1
        return real(brand, vendors)

    monkeypatch.setattr(pr, "match_brand", counting)
    pr.resolve_all({f"p{i}": "NOGA" for i in range(50)}, [], VENDORS)
    assert calls["n"] == 1


# ── the boundary: the purchase side never sees a brand ───────────────────────
def test_dominant_vendor_reads_only_bills():
    ...
    which a brand could reach it, which is why the separation survives the next
    screen that needs a vendor."""
    assert "brands" not in pr.dominant_vendor.__code__.co_varnames
...
def test_no_brand_key_can_ever_appear_in_the_purchase_side_answer():
    ...
    provably free of brand inference."""
    got = pr.dominant_vendor([_purchase("p1", "v_ken", 5.0),
                              _purchase("p2", "v_noga", 5.0)])
    assert not any(v.startswith(pr.BRAND_PREFIX) for v in got.values())


# ── naming, and reporting what the attribution is worth ──────────────────────
def test_brand_principals_are_named_where_the_vendor_table_cannot_name_them():
    resolved = pr.resolve_all({"p1": "Emuge Franken", "p2": "NOGA"}, [], VENDORS)
    names = pr.names_of(resolved, VENDORS)
    assert names["brand:emugefranken"] == "Emuge Franken"
...
def test_coverage_is_reported_in_money_as_well_as_items():
    ...
    assert report["from_bill"] == 1 and report["from_brand"] == 1
...
def test_the_brand_only_share_counts_principals_no_bill_corroborates():
    ...
    # Both came from a brand; only Emuge has no vendor row behind it.
    assert report["revenue_from_brand"] == pytest.approx(1000.0)
    assert report["brand_only_share"] == pytest.approx(0.25)
...
def test_a_book_that_has_not_traded_has_an_unknown_share_not_a_perfect_one():
    ...
    assert report["brand_only_share"] is None
...
def client():
    ...
    s.add_all([
        models.Product(product_id="p-billed", organization_id=org,
                       ...
                       brand="YG1", active=True, source_ref={}),
        models.Product(product_id="p-branded", organization_id=org,
                       external_id="e2", name="Old stock holder", hsn="84662000",
                       brand="Kennametal India Pvt Ltd", active=True,
                       source_ref={}),
    ...
    # bought before the sync window, which the brand is the only witness to.
    ...
    for pid, revenue in (("p-billed", "500"), ("p-branded", "300"),
                         ("p-neither", "200")):
...
def test_the_catalogue_says_which_evidence_placed_each_supplier(client):
    ...
    # The brand named a vendor this book has a record for, so it resolves to
    # that vendor's name rather than to the string on the item.
    assert by_id["p-branded"]["supplier"] == "Kennametal India Limited"
    assert by_id["p-branded"]["supplier_source"] == pr.BY_BRAND

    assert by_id["p-neither"]["supplier"] is None
    assert by_id["p-neither"]["supplier_source"] == pr.BY_NOTHING


def test_the_brand_is_carried_through_to_the_screen_that_corrects_it(client):
    head = _auth(client)
    listing = client.get("/api/v1/insight/catalogue?unplaced_only=false",
                         headers=head).json()
    row = next(i for i in listing["items"] if i["product_id"] == "p-branded")
    assert row["brand"] == "Kennametal India Pvt Ltd"
...
def test_every_screen_built_on_this_reports_what_it_could_attribute(client):
    ...
    for path in ("/api/v1/insight/catalogue?unplaced_only=false",
        ...
        assert body["principals"]["from_brand"] == 1, path
        assert body["principals"]["unattributed"] == 1, path


def test_a_purchase_target_counts_bills_only_and_no_brand_reaches_it(client):
    """The number that has to agree with the principal's own statement.

    ``p-branded`` is attributed to Kennametal by its brand and has sold ₹300,
    but no bill bought it. A purchase-basis target measures what this book
    *paid* Kennametal — ₹1,000 — and the brand-attributed sale must not appear
    in it, or the figure drifts from the invoices Kennametal actually raised
    and the one that is wrong at year end is ours.
    """
    head = _auth(client)
    ...
    # The sales side did see the brand, which is the whole point of the split.
